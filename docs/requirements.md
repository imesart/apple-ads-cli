...
- **`--from-json` excludes shortcut flags**: For `adgroups create`/`adgroups update` and `keywords create`/`keywords update`, `--from-json` rejects shortcut input flags (e.g. `--status`, `--default-bid`, `--cpa-goal`, `--text`, `--match-type`) with a usage error so silently-ignored flags can't mask user intent. Exception: `adgroups update --merge` is the documented overlay mode and accepts `--from-json` together with shortcuts (shortcuts win).
