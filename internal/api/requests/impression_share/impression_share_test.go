...
func TestAllRequests(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "Get",
			req:    GetRequest{ReportID: "rpt-1"},
			method: "GET",
			path:   "/custom-reports/rpt-1",
		},
		{
			name:     "List",
...
func TestGetRequest_PathIncludesID(t *testing.T) {
	req := GetRequest{ReportID: "abc"}
	if got := req.Path(); got != "/custom-reports/abc" {
		t.Errorf("Path() = %q, want %q", got, "/custom-reports/abc")
	}
}
