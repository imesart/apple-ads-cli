...
func TestAllRequests(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "Get",
			req:    GetRequest{BudgetOrderID: "123"},
			method: "GET",
			path:   "/budgetorders/123",
		},
		{
			name:     "List",
		...
		{
			name:    "Update",
			req:     UpdateRequest{BudgetOrderID: "123", RawBody: body},
			method:  "PUT",
			path:    "/budgetorders/123",
			hasBody: true,
		},
...
func TestGetRequest_PathIncludesID(t *testing.T) {
	req := GetRequest{BudgetOrderID: "abc"}
	if got := req.Path(); got != "/budgetorders/abc" {
		t.Errorf("Path() = %q, want %q", got, "/budgetorders/abc")
	}
}
